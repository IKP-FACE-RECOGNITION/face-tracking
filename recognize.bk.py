        ...
        for i in range(len(tracking_bboxes)):
            for j in range(len(detection_bboxes)):
                ...
                if mapping_score > 0.9:
                    ...
                    url = "http://127.0.0.1:8000/recognize-face/?model_name=SFace&detector_backend=yunet&distance_metric=cosine&align=true"
                    files = {
                        'file': ('result.jpg', img_bytes, 'image/jpeg')
                        }
                    res = await call_deepface_service(url,files)
                    print("Response: ",res)
                    if res.get("user_name") != "" | len(res.get("result"))>0: 
...
def main():
    ...
    # tracking(detector, config_tracking)
    recognize(detector, config_tracking)
